...
model_p = Path(config['model']['model_path']).expanduser()
label_p = Path(config['model']['label_map_path']).expanduser()

if not model_p.is_file():
    logger.error(f"Model file not found: {model_p}")
if not label_p.is_file():
    logger.error(f"Label map file not found: {label_p}")

self.gesture_recognizer = GestureRecognizer(
    model_path=str(model_p),
    label_map_path=str(label_p)
)
...
class GestureOverlayTester:
    ...
    def __init__(self, config):
        ...
        self.gesture_recognizer = GestureRecognizer(
            ...
            label_map_path=config['model']['label_map_path']
        )
        self.running = False
        self.cap = None
